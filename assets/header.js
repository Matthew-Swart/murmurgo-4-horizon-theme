...
class HeaderComponent extends Component {
  ...
  #resizeObserver = new ResizeObserver(([entry]) => {
    ...
    // Check if the new height is different from the current one to avoid infinite loops/flickering
    const currentHeight = parseInt(document.body.style.getPropertyValue('--header-height') || '0', 10);
    
    // Only update if the height has changed by more than 1px to prevent flickering
    if (Math.abs(roundedHeaderHeight - currentHeight) > 1) {
